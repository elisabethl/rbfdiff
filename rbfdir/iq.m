function [phi]=iq(epsil,r,nprime,dim)
%
...
elseif nprime(1:2)=='m2'
  phi = 8*epsil.^4.*sq(r(:,:,dim(1)+1)).*sq(r(:,:,dim(2)+1))./tmp.^3; 
...
elseif nprime(1:2)=='m3'
  ...
  if ndim==2 % dx_i^2 dx_j
      ...
      phi = -48*epsil.^6.*sq(r(:,:,d1+1)).^2.*sq(r(:,:,d2+1))./tmp.^4 + ...
             8*epsil.^4.*sq(r(:,:,d2+1))./tmp.^3;
      
  elseif ndim==3 % dx_i dx_j dx_k
      phi = -48*epsil.^6.*sq(r(:,:,dim(1)+1)).*sq(r(:,:,dim(2)+1)).* ...
            sq(r(:,:,dim(3)+1))./tmp.^4;
  else
...
elseif nprime(1:2)=='m4'
  ...
  if ndim==2
      if dim(1)==dim(2) & dim(3)==dim(4) % Two derivatives on each coordinate
          phi = 384*epsil.^8.*sq(r(:,:,dim(1)+1)).^2.* ...
                             sq(r(:,:,dim(3)+1)).^2./tmp.^5 - ...
                  48*epsil.^6.*(sq(r(:,:,dim(1)+1)).^2 + ...
                               sq(r(:,:,dim(3)+1)).^2)./tmp.^4 +  ...
                    8*epsil.^4./tmp.^3;
          ...
          phi = 384*epsil.^8.*sq(r(:,:,d1+1)).^3.*sq(r(:,:,d2+1))./tmp.^5 - ...
                 144*epsil.^6.*sq(r(:,:,d1+1)).*sq(r(:,:,d2+1))./tmp.^4;
  ...
  elseif ndim==3 % One coordinate has two derivatives
      ...
      phi = 384*epsil.^8.*sq(r(:,:,d1+1)).^2.*sq(r(:,:,d2+1)).* ...
                          sq(r(:,:,d3+1))./tmp.^5 -  ...
              48*epsil.^6.*sq(r(:,:,d2+1)).*sq(r(:,:,d3+1))./tmp.^4;
      
  elseif ndim==4 % All have one derivative
      phi = 384*epsil.^8.*sq(r(:,:,dim(1)+1)).*sq(r(:,:,dim(2)+1)).* ...
                          sq(r(:,:,dim(3)+1)).*sq(r(:,:,dim(4)+1))./tmp.^5;
  else
      error('Error in input argument dim to function mq')
  end    
else
