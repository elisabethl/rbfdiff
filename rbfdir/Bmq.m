function [phi]=Bmq(epsil,r,nprime,dim)
%
...
tmp = 1+(epsil.*sq(r(:,:,1))).^6;

if nprime(1)=='0'
  phi = sqrt(tmp);

elseif nprime(1)=='1'
  phi = 3*epsil.^6.*sq(r(:,:,dim+1)).*sq(r(:,:,1)).^4./sqrt(tmp);

elseif nprime(1)=='2'
  phi = (3*epsil.^6.*sq(r(:,:,1)).^4 + ...
	 12*epsil.^6.*sq(r(:,:,dim+1)).^2.*sq(r(:,:,1)).^2)./sqrt(tmp) - ...
         9*epsil.^12.*sq(r(:,:,dim+1)).^2.*sq(r(:,:,1)).^8.*tmp.^-1.5;
...
elseif nprime(1:2)=='m2'
  phi = 12*epsil.^6.*sq(r(:,:,dim(1)+1)).*sq(r(:,:,dim(2)+1)).*sq(r(:,:,1)).^2./sqrt(tmp) - ...
        9*epsil.^12.*sq(r(:,:,dim(1)+1)).*sq(r(:,:,dim(2)+1)).*sq(r(:,:,1)).^8.*tmp.^-1.5; 
