function [phi]=bump(epsil,r,nprime,dim)
%
...
% Make sure every value above r becomes zero
r(:,:,1) = double(sq(r(:,:,1))>=1) + sq(r(:,:,1));

% Ensuring tmp = 0 for r >= 1 and all derivatives = 0 for r = 1
tmpPw = abs(1./(1-sq(r(:,:,1)).^2)); 
tmp = (sq(r(:,:,1))<1).*exp(-tmpPw);

if nprime(1)=='0'
  phi = tmp;

elseif nprime(1)=='1'
  phi = -2.*sq(r(:,:,dim+1)).*(tmpPw.^2).*tmp;

elseif nprime(1)=='2'
  phi = ((sq(r(:,:,dim+1)).^2).*(4.*tmpPw.^4 - 8.*tmpPw.^3) - 2.*tmpPw.^2).*tmp;

elseif nprime(1)=='L' && length(nprime)==1
  phi = ((sq(r(:,:,1)).^2).*(4.*tmpPw.^4 - 8.*tmpPw.^3) - 2.*nd.*tmpPw.^2).*tmp;

elseif nprime(1:2)=='m2'
  phi = sq(r(:,:,dim(1)+1)).*sq(r(:,:,dim(2)+1)).*(4.*tmpPw.^4 - 8.*tmpPw.^3).*tmp; 
