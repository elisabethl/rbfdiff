...
%
clear
N = 2000; % The number of node points in the domain
Ne = min(400,2*N); % The number of evaluation points
epvec = logspace(-2,log10(2),50); % To get error as a function of ep
...
for j=1:length(epvec)
    ...
    % Old errors
    %
    maxerr2(j) = max(abs(A2*uk-fun(xe(:,1),xe(:,2))));
    errx2(j) = max(abs(G2{1}*uk-funx(xe(:,1),xe(:,2))));
    erry2(j) = max(abs(G2{2}*uk-funy(xe(:,1),xe(:,2))));
    errxx2(j) = max(abs(H2{1,1}*uk-funxx(xe(:,1),xe(:,2))));
    errxy2(j) = max(abs(H2{1,2}*uk-funxy(xe(:,1),xe(:,2))));
    erryy2(j) = max(abs(H2{2,2}*uk-funyy(xe(:,1),xe(:,2))));
    errL2(j) = max(abs(L2*uk-funxx(xe(:,1),xe(:,2))-funyy(xe(:,1),xe(:,2))));
end

figure
H1=loglog(epvec,maxerr,epvec,errx,epvec,erry,epvec,errxx,epvec,errxy,epvec,erryy,epvec,errL);
hold on
H2=loglog(epvec,maxerr2,epvec,errx2,epvec,erry2,epvec,errxx2,epvec,errxy2,epvec,erryy2,epvec,errL2);
...
for k=1:length(H1)
    set(H1(k),'LineStyle','-')
    col=get(H1(k),'Color')
    set(H2(k),'LineStyle','none','Marker','.','Color',col)
end    
